...
public class DragController {
    ...
    protected static final float MAX_FLING_DEGREES = 35f;
    ...
    protected final ArrayList<DropTarget> mDropTargets = new ArrayList<DropTarget>();
    protected final ArrayList<DragListener> mListeners = new ArrayList<DragListener>();
    protected DropTarget mFlingToDeleteDropTarget;
    //protected final ArrayList<DropTarget> mFlingToDeleteDropTargets = new ArrayList<DropTarget>();
    ...
    protected int mScrollState = SCROLL_OUTSIDE_ZONE;
    ...
    protected int mLastTouch[] = new int[2];
    protected long mLastTouchUpTime = -1;
    protected int mDistanceSinceScroll = 0;

    protected int mTmpPoint[] = new int[2];
    protected Rect mDragLayerRect = new Rect();
    ...
    public DragController(Launcher launcher) {
        ...
        mScrollZone = resManager.getDimensionPixelSize(IResConfigManager.DIM_SCROLL_ZONE);
        //r.getDimensionPixelSize(R.dimen.scroll_zone);
        mFlingToDeleteThresholdVelocity = resManager.getIntegerWithDensity(IResConfigManager.CONFIG_FLING_TO_DELETE_MIN_VELOCITY);
    ...
    public void startDrag(Bitmap b, int dragLayerX, int dragLayerY,
        ...
        if (Util.DEBUG_DRAG) {
            Util.Log.d(TAG, "startDrag: dragLayerX = " + dragLayerX + ", dragLayerY = " + dragLayerY
    ...
    public void cancelDrag() {
        if (Util.ENABLE_DEBUG) {
        	Util.Log.d(TAG, "cancelDrag: mDragging = " + mDragging + ", mLastDropTarget = " + mLastDropTarget);
        }
    ...
    public void onAppsRemoved(List<String> packageNames, Context context) {
    ...
    protected void endDrag() {
        if (Util.DEBUG_DRAG) {
        	Util.Log.d(TAG, "endDrag: mDragging = " + mDragging + ", mDragObject = " + mDragObject);
        }
        if (mDragging) {
            mDragging = false;
            clearScrollRunnable();
     ...
     */
    protected int[] getClampedDragLayerPos(float x, float y) {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (Util.DEBUG_MOTION) {
        	Util.Log.d(TAG, "onInterceptTouchEvent: action = " + action + ", mDragging = " + mDragging
    ...
    protected void handleMoveEvent(int x, int y) {
        ...
        if (Util.DEBUG_DRAG && (mLastDropTarget != dropTarget)) {
        	Util.Log.v(TAG, "handleMoveEvent: x = " + x + ", y = " + y + ", dragView = "
                    + mDragObject.dragView + ", dragX = " + mDragObject.x + ", dragY = " + mDragObject.y
                    +",mScrollState:"+mScrollState
                    +",mScrollZone:"+mScrollZone
                    +",mScrollWidth:"+mScrollView.getWidth());
        }
        ...
        if (x < mScrollZone) {
            if (mScrollState == SCROLL_OUTSIDE_ZONE) {
                ...
                } else if (Util.DEBUG_DRAG){
                    Util.Log.d(TAG, "handleMoveEvent(3): SCROLL_LEFT, onEnterScrollArea fail==");
                }
            }
            ...
            if (mScrollState == SCROLL_OUTSIDE_ZONE) {
                ...
                if (mDragScroller.onEnterScrollArea(x, y, SCROLL_RIGHT)) {
                    ...
                    Util.Log.d(TAG, "handleMoveEvent(4): SCROLL_RIGHT, onEnterScrollArea fail==");
                }
            }
        } else {
            clearScrollRunnable();
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (Util.DEBUG_MOTION) {
        	Util.Log.d(TAG, "onTouchEvent: action = " + action + ", dragLayerX = " + dragLayerX
     ...
     */
    protected PointF isFlingingToDelete(DragSource source) {
    ...
    protected void dropOnFlingToDeleteTarget(float x, float y, PointF vel) {
        final int[] coordinates = mCoordinatesTemp;
        ...
        //if (mLastDropTarget != null && mFlingToDeleteDropTargets.contains(mLastDropTarget)) {
            mLastDropTarget.onDragExit(mDragObject);
        }
    ...
    protected void drop(float x, float y) {
        ...
        mDragObject.y = coordinates[1];
        if (Util.DEBUG_DRAG) {
        	Util.Log.d(TAG, "drop: x = " + x + ", y = " + y + ", mDragObject.x = " + mDragObject.x
