...
public class Alarm implements Runnable{
    ...
    public void setAlarm(long millisecondsInFuture) {
        ...
        if (Util.ENABLE_DEBUG) {
        	Util.Log.d(TAG, "setAlarm: currentTime = " + currentTime + ", millisecondsInFuture = "
    ...
    public void cancelAlarm() {
        if (Util.ENABLE_DEBUG) {
        	Util.Log.d(TAG, "Cancel alarm here: mAlarmListener = " + mAlarmListener);
        }
    ...
    public void run() {
        ...
        if (mAlarmTriggerTime != 0) {
            ...
            if (Util.ENABLE_DEBUG) {
            	Util.Log.d(TAG, "run: mAlarmTriggerTime = " + mAlarmTriggerTime + ", currentTime = "
