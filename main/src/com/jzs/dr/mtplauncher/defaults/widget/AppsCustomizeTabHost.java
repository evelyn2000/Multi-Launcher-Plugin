...
import com.jzs.dr.mtplauncher.sjar.ctrl.AppsCustomizeTabKeyEventListener;
...
public class AppsCustomizeTabHost extends TabHost implements IAppsCustomizePanel,
...
//	private static final String WIDGETS_SHORTCUT_TAB_TAG = "WIDGETS_SHORTCUT";
    ...
    private PageViewIndicator mAppsCustomizePaneIndicator;
    //private PageViewIndicator mAppsCustomizePaneWidgetsIndicator;
     ...
     * tabs manually).
     */
    public void setContentTypeImmediate(int type) {
    ...
    public void selectAppsTab() {
        if (Util.ENABLE_DEBUG) {
        	Util.Log.d(TAG, "selectAppsTab.");
        }

        setContentTypeImmediate(AppsCustomizePagedView.CONTENTTYPE_APPS_ALL);
    }

    public void selectWidgetsTab() {
        if (Util.ENABLE_DEBUG) {
        	Util.Log.d(TAG, "selectWidgetsTab.");
        }

        setContentTypeImmediate(AppsCustomizePagedView.CONTENTTYPE_WIDGETS);
    }
    
    public void selectAppsTabDelay() {
        if (Util.ENABLE_DEBUG) {
        	Util.Log.d(TAG, "selectAppsTab.");
        }

        setContentType(AppsCustomizePagedView.CONTENTTYPE_APPS_ALL);
    }
    
    public void selectWidgetsTabDelay() {
        if (Util.ENABLE_DEBUG) {
        	Util.Log.d(TAG, "selectWidgetsTab.");
        }

        setContentType(AppsCustomizePagedView.CONTENTTYPE_WIDGETS);
    }
    ...
    protected void onFinishInflate() {
        ...
        PageViewIndicator indicator = (PageViewIndicator)mContent.findViewById(R.id.paged_view_indicator);
        if(indicator != null){
        	indicator.initial(mAppsCustomizePane.getCurrentPage(), mAppsCustomizePane.getPageCount());
        	mAppsCustomizePane.setPageSwitchListener(indicator);
        }
        mAppsCustomizePaneIndicator = indicator;
        
//        indicator = (PageViewIndicator)mContent.findViewById(R.id.paged_view_indicator_widgets);
//        if(indicator != null){
//        	indicator.initial(mAppsCustomizePaneWidgets.getCurrentPage(), mAppsCustomizePaneWidgets.getPageCount());
//        	mAppsCustomizePaneWidgets.setPageSwitchListener(indicator);
//        }
//        mAppsCustomizePaneWidgetsIndicator = indicator;
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (Util.DEBUG_LAYOUT) {
        	Util.Log.d(TAG, "onMeasure end: remeasureTabWidth = " + remeasureTabWidth
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (Util.ENABLE_DEBUG) {
        	Util.Log.d(TAG, "onTouchEvent: action = " + event.getAction() + ", y = " + event.getY());
        }
...
//    	}else if(mAppsCustomizePaneWidgets != null && mAppsCustomizePaneWidgets.getVisibility() == View.VISIBLE){
...
//    	}
    ...
    private void reloadCurrentPage() {
        ...
        if(mAppsCustomizePaneWidgets != null && mAppsCustomizePaneWidgets.getVisibility() == View.VISIBLE){
    ...
    public void onTabChanged(String tabId) {
        ...
        if (Util.ENABLE_DEBUG) {
        	Util.Log.d(TAG, "onTabChanged: tabId = " + tabId + ", type = " + type);
        }
        ...
        post(new Runnable() {
            ...
            public void run() {
            	...
            	if ((mAppsCustomizePane.getMeasuredWidth() <= 0 ||
                    ...
                    reloadCurrentPage();
                    Util.Log.d("QsLog", "reloadCurrentPage");
                    return;
                }
                ...
                appCustomizePane.getVisiblePages(visiblePageRange);
            	...
            	if(true){
	                ...
	                ObjectAnimator inAnim = LauncherAnimUtils.ofFloat(appCustomizePane, "alpha", 0f, 1f);
	                inAnim.addListener(new AnimatorListenerAdapter() {
						...
						public void onAnimationStart(Animator animation) {
	                    	...
	                    	if(type == AppsCustomizePagedView.CONTENTTYPE_APPS_ALL){
	                    		selectAppsTabDelay();
	                    		//Util.Log.d("QsLog", "CONTENTTYPE_APPS_ALL");
	                    	}
	                    	else if(type == AppsCustomizePagedView.CONTENTTYPE_WIDGETS){
	                    		selectWidgetsTabDelay();
	                    		//Util.Log.d("QsLog", "CONTENTTYPE_WIDGETS");
	                    	}
						}
	                ...
	                animSet.play(inAnim);//.after(outAnim);
    ...
    public int getContentTypeForTabTag(String tag) {
        if (tag.equals(APPS_TAB_TAG)) {
            return AppsCustomizePagedView.CONTENTTYPE_APPS_ALL;
    ...
    public void reset() {
        if (mInTransition) {
            ...
            if(mAppsCustomizePaneWidgets != null) mAppsCustomizePaneWidgets.reset();
        }
    }
    ...
    public void onLauncherTransitionPrepare(Launcher l, boolean animated, boolean toWorkspace) {
        if (Util.DEBUG_LAYOUT) {
            Util.Log.d(TAG, "onLauncherTransitionPrepare: toWorkspace = " + toWorkspace
...
//            }
    ...
    @Override
    public void onLauncherTransitionStart(Launcher l, boolean animation, boolean toWorkspace) {
        if (Util.ENABLE_DEBUG) {
            Util.Log.d(TAG, "onLauncherTransitionStart: l = " + l + ", animated = " + animation + ", toWorkspace = "
                    + toWorkspace);
        }
        if (animation) {
    ...
    public void onLauncherTransitionEnd(Launcher l, boolean animated, boolean toWorkspace) {
        if (Util.ENABLE_DEBUG) {
            Util.Log.d(TAG, "onLauncherTransitionEnd: l = " + l + ", animated = " + animated + ", toWorkspace = "
...
//            if (!Launcher.isScreenLarge()) {
//                //mAppsCustomizePane.hideScrollingIndicator(false);
//                //if(mAppsCustomizePaneWidgets != null) mAppsCustomizePaneWidgets.hideScrollingIndicator(false);
//            }
    ...
    public void onResume() {
        ...
        if (getVisibility() == VISIBLE) {
            ...
            if(mAppsCustomizePaneWidgets != null && mAppsCustomizePaneWidgets.getVisibility() == View.VISIBLE){
	            ...
	            mAppsCustomizePaneWidgets.invalidateAppPages(currentPage, true);
	            
            } else {
                int currentPage = mAppsCustomizePane.getCurrentPage();
	            mAppsCustomizePane.invalidateAppPages(currentPage, true);
            }
        }
    }
