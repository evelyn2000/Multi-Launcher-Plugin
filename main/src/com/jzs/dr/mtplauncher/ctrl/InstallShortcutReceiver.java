...
package com.jzs.dr.mtplauncher.ctrl;
...
public class InstallShortcutReceiver extends BroadcastReceiver {
    ...
    private static LauncherHelper getLauncherHelper(Context context){
    	LauncherApplicationMain app = (LauncherApplicationMain) context.getApplicationContext();
    	return app.getLauncherHelper();
    }

    public void onReceive(Context context, Intent data) {
        if (Util.ENABLE_DEBUG) {
        	Util.Log.d(TAG, "onReceive: received intent action: " + data.getAction());
        }
        if (!IGlobalStaticFunc.ACTION_INSTALL_SHORTCUT.equals(data.getAction())) {
            return;
        }
    ...
    private static void installShortcutSingle(Context context, Intent data) {
        ...
        LauncherHelper launcher = getLauncherHelper(context);
        ...
        if (Util.ENABLE_DEBUG) {
        	Util.Log.d(TAG, "installShortcutSingle: data = " + data + ", name = " + name
                    + ", intent = " + intent);
        }
        
        // Queue the item up for adding if launcher has not loaded properly yet
        boolean launcherNotLoaded = launcher.getWorkspaceCellCountX() <= 0
                || launcher.getWorkspaceCellCountY() <= 0;

        PendingInstallShortcutInfo info = new PendingInstallShortcutInfo(data, name, intent);
        if (mUseInstallQueue || launcherNotLoaded) {
            mInstallQueue.add(info);
            if (Util.ENABLE_DEBUG) {
            	Util.Log.d(TAG, "installShortcutSingle: Add the install process into queue " + mInstallQueue.size());
            }
        } else {
    ...
    private static void installShortcutStep(Context context, Intent data) {
        Intent intent = data.getParcelableExtra(Intent.EXTRA_SHORTCUT_INTENT);
        if (intent == null) {
            Util.Log.e(TAG, "installShortcutStep: Intent is null!");
            return;
        }

        int totalNumber = data.getIntExtra(EXTRA_SHORTCUT_TOTAL_NUMBER, 0);
        if (totalNumber < 1) {
            Util.Log.e(TAG, "installShortcutStep: total number is smaller than 1!");
            return;
        }

        int stepNumber = data.getIntExtra(EXTRA_SHORTCUT_STEP_NUMBER, 0);
        if (stepNumber < 1 || stepNumber > totalNumber) {
            Util.Log.e(TAG, "installShortcutStep: Step number is wrong!");
            return;
        }
        ...
        if (uri != null) {
            ...
            if (pkgName == null) {
                if (Util.ENABLE_DEBUG) {
                	Util.Log.d(TAG, "installShortcutStep: Package name is null!");
                }
                return;
            }
        }
        ...
        if (Util.ENABLE_DEBUG) {
        	Util.Log.d(TAG, "installShortcutStep: data = " + data + ", name = " + pkgName
        ...
        if (mUseInstallQueue || launcherNotLoaded) {
            ...
            if (name == null) {
                try {
                    ...
                    if (Util.ENABLE_DEBUG) {
                    	Util.Log.d(TAG, "installShortcutStep: Activity name is not found!");
                    }
                    return;
                }
            }
            ...
            if (stepNumber == totalNumber) {
                ...
                for (Intent eachIntent : intentArray) {
                    if (eachIntent == null) {
                        Util.Log.e(TAG, "installShortcutStep: IntentArray has null intent!");
                        clearMaps(pkgName);
                        return;
                    }
                }
    ...
    private static void installShortcutArray(Context context, Intent data) {
        ...
        for (int i = 0; i < size; i++) {
            if ((Intent) intentArray[i] == null) {
                if (Util.ENABLE_DEBUG) {
                	Util.Log.e(TAG, "installShortcutArray: intent is null with " + i);
                }
                return;
            }
        ...
        if (iconResourceArray != null && iconResourceArray.length != size) {
            if (Util.ENABLE_DEBUG) {
            	Util.Log.e(TAG, "installShortcutArray: icon resource array is not null but "
                        + "the size not match!");
            }
            return;
        }
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (mUseInstallQueue || launcherNotLoaded) {
                if (Util.ENABLE_DEBUG) {
                	Util.Log.d(TAG, "installShortcutArray: Add into Install Queue!");
                }
...
//        SharedPreferences sp = context.getSharedPreferences(spKey, Context.MODE_PRIVATE);
        ...
        SharedPreferences sp = sharedManager.getSharedPreferences();
        ...
        if (Util.ENABLE_DEBUG) {
        	Util.Log.d(TAG, "processInstallShortcut pendingInfo = " + pendingInfo + ", data = "
        ...
        synchronized (launcher) {
            ...
            // ceil(i/2f)*(-1)^(i-1))
            final int screenCount = sharedManager.getWorkspaceScreenCount();
            final int screen = sharedManager.getWorkspaceDefaultScreen();
        ...
        if (result[0] == INSTALL_SHORTCUT_NO_SPACE 
            ...
            InstallShortcutHelper.decreaseInstallingCount(launcher, false);
        }
        /// M: }@
    }
    ...
    private static void processInstallShortcutArray(Context context, Intent[] dataArray) {
        ...
        //String spKey = LauncherApplication.getSharedPreferencesKey();
    	ISharedPrefSettingsManager sharedManager = app.getLauncherHelper().getSharedPrefSettingsManager();
        SharedPreferences sp = sharedManager.getSharedPreferences();//context.getSharedPreferences(spKey, Context.MODE_PRIVATE);
        ...
        for (int j = 0; j < dataArray.length; j++) {
            ...
            if (Util.ENABLE_DEBUG) {
            	Util.Log.d(TAG, "processInstallShortcutArray: data = " + data + ", intent = "
                        + intent + ", name = " + name);
            }
            ...
            synchronized (app) {
                ...
                // ceil(i/2f)*(-1)^(i-1))
                final int screenCount = sharedManager.getWorkspaceScreenCount();
                final int screen = sharedManager.getWorkspaceDefaultScreen();//Launcher.DEFAULT_SCREEN;
                for (int i = 0; i < (2 * screenCount) + 1 && !found; ++i) {
                    int si = screen + (int) ((i / 2f) + 0.5f) * ((i % 2 == 1) ? 1 : -1);
                    if (0 <= si && si < screenCount) {
                        found = installShortcut(context, data, items, name, intent, si, exists, sp,
            ...
            case INSTALL_SHORTCUT_ADD_FAIL:
                ...
                InstallShortcutHelper.decreaseInstallingCount(app.getLauncherHelper(), false);
                break;
            ...
            if (Util.ENABLE_DEBUG) {
            	Util.Log.d(TAG, "processInstallShortcutArray: result is " + result[0]);
            }
        }
    ...
    private static boolean installShortcut(Context context, Intent data, ArrayList<ItemInfo> items,
        ...
        if (Util.ENABLE_DEBUG) {
        	Util.Log.d(TAG, "installShortcut data = " + data + ", items = " + items
        ...
        if (findEmptyCell(context, items, tmpCoordinates, screen)) {
            if (intent != null) {
                ...
                if (duplicate || !shortcutExists) {
                    ...
                    int newAppsScreen = sharedPrefs.getInt(IGlobalStaticFunc.NEW_APPS_PAGE_KEY, screen);
                    Set<String> newApps = new HashSet<String>();
                    if (newAppsScreen == screen) {
                        newApps = sharedPrefs.getStringSet(IGlobalStaticFunc.NEW_APPS_LIST_KEY, newApps);
                    }
                    ...
                    new Thread("setNewAppsThread") {
                        public void run() {
                            synchronized (savedNewApps) {
                                sharedPrefs.edit().putInt(IGlobalStaticFunc.NEW_APPS_PAGE_KEY, screen)
                                        .putStringSet(IGlobalStaticFunc.NEW_APPS_LIST_KEY, savedNewApps).commit();
                            }
                        }
                    }.start();
                    ...
                    //LauncherApplicationMain app = (LauncherApplicationMain) context.getApplicationContext();
                    LauncherHelper launcher = getLauncherHelper(context);
                    ShortcutInfo info = launcher.getModel().addShortcut(data,
                            LauncherSettings.Favorites.CONTAINER_DESKTOP, screen,
                            tmpCoordinates[0], tmpCoordinates[1], true);
                    if (info == null) {
                        result[0] = INSTALL_SHORTCUT_ADD_FAIL;
                        Util.Log.e(TAG, "InstallShortcut Failed: Due to ShortcutInfo is null");
                        return false;
                    }
                    sItemsAddingToDatabase.add(info);
                    result[0] = INSTALL_SHORTCUT_SUCCESSFUL;
                    if (Util.ENABLE_DEBUG) {
                    	Util.Log.d(TAG, "InstallShortcut Successfully: Install the " + info.title);
                    }
                } else {
                    Util.Log.e(TAG, "InstallShortcut Failed: Already Exist!");
                    result[0] = INSTALL_SHORTCUT_IS_DUPLICATE;
                }
            ...
            Util.Log.e(TAG, "InstallShortcut Failed: No Space!");
    ...
    private static boolean findEmptyCell(Context context, ArrayList<ItemInfo> items, int[] xy,
        ...
        final int xCount = launcher.getWorkspaceCellCountX();
        final int yCount = launcher.getWorkspaceCellCountY();
