...
public class UninstallShortcutReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent data) {
        if (!ACTION_UNINSTALL_SHORTCUT.equals(data.getAction())) {
            Util.Log.d(TAG, "Receive " + data.getAction() + " in UninstallShortcutReceiver.");
            return;
        }

        PendingUninstallShortcutInfo info = new PendingUninstallShortcutInfo(data);
        if (Util.ENABLE_DEBUG) {
        	Util.Log.d(TAG, "onReceive data = " + data + ", info = " + info);
        }
    ...
    private static void processUninstallShortcut(Context context,
        ...
        SharedPreferences sharedPrefs = app.getLauncherHelper().getSharedPrefSettingsManager().getSharedPreferences();//context.getSharedPreferences(spKey, Context.MODE_PRIVATE);

        final Intent data = pendingInfo.data;
        if (Util.ENABLE_DEBUG) {
        	Util.Log.d(TAG, "processUninstallShortcut pendingInfo = " + pendingInfo
                    + ", data = " + data);
        }
    ...
    private static void removeShortcut(Context context, Intent data,
        ...
        if (Util.ENABLE_DEBUG) {
        	Util.Log.d(TAG, "onReceive: data = " + data + ", intent = " + intent + ", name = "
            ...
            try {
                while (c.moveToNext()) {
                    try {
                        ...
                        Util.Log.w(TAG, "URISyntaxException happened when removeShortcut.");
                    }
                }
            } finally {
                c.close();
                ...
                new Thread("setNewAppsThread-remove") {
                    public void run() {
                        synchronized (savedNewApps) {
                            ...
                            editor.putStringSet(IGlobalStaticFunc.NEW_APPS_LIST_KEY,
                                    savedNewApps);
                            if (savedNewApps.isEmpty()) {
                                // Reset the page index if there are no more items
                                editor.putInt(IGlobalStaticFunc.NEW_APPS_PAGE_KEY, -1);
                            }
                            editor.commit();
                        }
                    }
