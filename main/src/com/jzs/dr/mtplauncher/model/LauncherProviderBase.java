...
public class LauncherProviderBase  extends ContentProvider implements ILauncherProvider{
	//extends LauncherProvider {
    private static final String TAG = "Jzs.LauncherProvider";
    ...
    static final Uri CONTENT_APPWIDGET_RESET_URI =
            Uri.parse("content://" + LauncherSettings.AUTHORITY + "/appWidgetReset");

    private DatabaseHelper mOpenHelper;
    
    @Override
    public boolean onCreate() {
    	//Util.Log.w(TAG, "LauncherProviderBase::setLauncherProvider()====");
    	LauncherApplicationMain app = (LauncherApplicationMain) getContext();
    	app.setLauncherProvider(this);
    ...
    private void sendNotify(Uri uri) {
        String notify = uri.getQueryParameter(LauncherSettings.PARAMETER_NOTIFY);
    ...
    synchronized public void loadDefaultFavoritesIfNecessary(int origWorkspaceResId) {
    	LauncherApplicationMain app = (LauncherApplicationMain) getContext();
    	SharedPreferences sp = app.getLauncherHelper().getSharedPreferences();
        //String spKey = SharedPrefSettingsManager.getSharedPreferencesKey();
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        //private ILauncherProviderExt mLauncherProviderExt;
        private LauncherHelper mLauncher;
        
        DatabaseHelper(Context context) {
            super(context, LauncherSettings.DATABASE_NAME, null, LauncherSettings.DATABASE_VERSION);
            mContext = context;
            mAppWidgetHost = new AppWidgetHost(context, Launcher.APPWIDGET_HOST_ID);

            LauncherApplicationMain app = (LauncherApplicationMain) context;      
            mLauncher = app.getLauncherHelper();
        	//mLauncherProviderExt = (ILauncherProviderExt)app.getLauncherPluginEntry().getExtHelper(LauncherPluginEntry.EXT_HELPER_TARGET_PROVIDER);
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            StringBuilder str = new StringBuilder("CREATE TABLE favorites (" +
            ...
            String extStr = "";
//            if(mLauncherProviderExt != null)
//            	extStr = mLauncherProviderExt.CreateDbTableExtString();
            if(!TextUtils.isEmpty(extStr)){
            	str.append(",").append(extStr);
            }
...
//                    ");");
        ...
        private int copyFromCursor(SQLiteDatabase db, Cursor c) {
            ...
            while (c.moveToNext()) {
                ...
                values.put(LauncherSettings.Favorites.LAUNCH_FREQ, c.getInt(freqIndex));
                values.put(LauncherSettings.Favorites.LAST_LAUNCH_TIME, c.getLong(lastTimeIndex));
                rows[i++] = values;
            }
            ...
            try {
                ...
                for (i = 0; i < numValues; i++) {
                    if (dbInsertAndCheck(this, db, LauncherSettings.TABLE_FAVORITES, null, rows[i]) < 0) {
                        return 0;
                    } else {
                        total++;
                    }
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            ...
            if (version != LauncherSettings.DATABASE_VERSION) {
                Log.w(TAG, "Destroying all old data.");
                db.execSQL("DROP TABLE IF EXISTS " + LauncherSettings.TABLE_FAVORITES);
                //db.execSQL("DROP TABLE IF EXISTS " + LauncherSettings.TABLE_SETTINGS);
                onCreate(db);
            }
        }
...
//                c = db.query(LauncherSettings.TABLE_FAVORITES,
...
//                                    db.update(LauncherSettings.TABLE_FAVORITES, values, updateWhere, null);
//                                }
//                            }
...
//        }
        ...
        private void convertWidgets(SQLiteDatabase db) {
            ...
            try {
                // Select and iterate through each matching widget
                c = db.query(LauncherSettings.TABLE_FAVORITES, new String[] { LauncherSettings.Favorites._ID, LauncherSettings.Favorites.ITEM_TYPE },
                        selectWhere, null, null, null, null);
...
//            int allAppsButtonRank = ILauncherProviderExt.ALL_APPS_BTN_RANK_INDEX_DEFAULT;// = mContext.getResources().getInteger(R.integer.hotseat_all_apps_index);
//            if(mLauncherProviderExt != null)
//            	allAppsButtonRank = mLauncherProviderExt.getAllAppsButtonRank();
            
            ISharedPrefSettingsManager shareManager = mLauncher.getSharedPrefSettingsManager();//.getResConfigManager();
            int allAppsButtonRank = shareManager.getAllAppsButtonRank();
//            if(allAppsButtonRank == ILauncherProviderExt.ALL_APPS_BTN_RANK_INDEX_DEFAULT)
//            	allAppsButtonRank = mContext.getResources().getInteger(R.integer.hotseat_all_apps_index);
            final IAppsManager appManager = (IAppsManager)mContext.getSystemService(IAppsManager.MANAGER_SERVICE);
			
			
            int i = 0;
            try {
            	
                XmlResourceParser parser = mLauncher.getResConfigManager().getXml(IResConfigManager.OTHER_WORKSPACE_DEFAULT_XML);// = mContext.getResources().getXml(workspaceResourceId);
//                if(mLauncherProviderExt != null)
//                	parser = mLauncherProviderExt.getDefaultWorkspaceResource();
//                if(parser == null)
//                	parser = mContext.getResources().getXml(workspaceResourceId);
                ...
                while (((type = parser.next()) != XmlPullParser.END_TAG ||
                    ...
                    if (TAG_FAVORITE.equals(name)) {
                        ...
                        if(!TextUtils.isEmpty(className) && !TextUtils.isEmpty(packageName)){
	                        if(appManager != null){
	                        	title = appManager.getTitle(packageName, className, 0);
	            			}
	                        values.put(LauncherSettings.Favorites.ICON_PACKAGE, packageName);
	                        values.put(LauncherSettings.Favorites.ICON_RESOURCE, className);
                        }
        ...
        private long addAppShortcut(SQLiteDatabase db, ContentValues values, TypedArray a,
            ...
            try {
                ...
                if (dbInsertAndCheck(this, db, LauncherSettings.TABLE_FAVORITES, null, values) < 0) {
                    return -1;
                }
        ...
        private boolean addAppWidget(SQLiteDatabase db, ContentValues values, ComponentName cn,
            ...
            try {
                ...
                values.put(LauncherSettings.Favorites.APPWIDGET_ID, appWidgetId);
                values.put(LauncherSettings.Favorites._ID, generateNewId());
                dbInsertAndCheck(this, db, LauncherSettings.TABLE_FAVORITES, null, values);
